...
#      (It's looking a bit different now from Inns' original!)3
#
...
from lib_tft144.lcdfonts import *
...
class TFT144:
    ...
    def __init__(self, gpio, spidev, CE, dc_pin, rst_pin=0, led_pin=0, orientation=ORIENTATION0,
                 isRedBoard= False, spi_speed=16000000):
        ...
        self.PINK = self.colour565(255, 127, 127)
        self.LIGHTBLUE = self.colour565(127, 127, 255)
        self.LIGHTGREEN = self.colour565(127, 255, 127)
        self.BLACK = self.colour565(0, 0, 0)
        self.WHITE = self.colour565(255, 255, 255)
        self.GREY = self.colour565(127, 127, 120)
    ...
    def write_data(self, data):
        ...
        if not isinstance(data, list):  # is it already a list?
            data = [data]
    ...
    def put_string(self, str, originx, y, fgcolor, bgcolor, font=3):
        x = originx
        fontW = self.fontDim[font][0]  # Width of a letter
        fontH = self.fontDim[font][1]  # Height of a letter
